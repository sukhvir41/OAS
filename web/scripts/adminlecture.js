...
var getLectures = function(){
	...
	var template = '<tr><td><a href="/OAS/admin/lectures/detaillecture?lectureId={{id}}">{{id}}</a></td><td>{{class}}</td><td>{{subject}}</td><td>{{count}}</td><td>{{date}}</td><td>{{present}}</td><td>{{absent}}</td></tr>';
	error.hide();
	success.hide();

	$.ajax({
		url: "/OAS/admin/ajax/searchlecture",
		dataType: "json",
		data: {
