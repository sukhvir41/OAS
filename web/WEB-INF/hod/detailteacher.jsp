...
<html>
    <head>
        ...
        <title>Teacher Details - HOD</title>   
                                    ...
                                    <c:if test="${!requestScope.teacher.unaccounted}">
                                        <c:choose>
                                            <c:when test="${requestScope.teacher.verified}">
                                                <a class="btn btn-danger" value="${requestScope.teacher.id}" href="/OAS/teacher/hod/teachers/deactivateteacher?teacherId=${requestScope.teacher.id}">Deverify</a>
                                            </c:when>
                                            <c:otherwise>
                                                <a class="btn btn-success" value="${requestScope.teacher.id}" href="/OAS/teacher/hod/teachers/activateteacher?teacherId=${requestScope.teacher.id}">Verify</a>
                                            </c:otherwise>
                                        </c:choose>
                                    </c:if>
                                    <c:choose>
                                    ...
                                    <div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="formModalLabel" aria-hidden="true" style="display: none;">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                ...
                                                <form class="form-horizontal mb-lg" action="/OAS/teacher/hod/teachers/deleteteacher" method="post">
                                    ...
                                    <div class="modal fade" id="unaccountTeacher" tabindex="-1" role="dialog" aria-labelledby="formModalLabel" aria-hidden="true" style="display: none;">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                ...
                                                <form class="form-horizontal mb-lg" action="/OAS/teacher/hod/teachers/unaccountteacher" method="post">
                                                    <div class="modal-body">
                                                        <input type="hidden" name="teacherId" value="${requestScope.teacher.id}">
                                    ...
                                    <table class="table">
                                        ...
                                        <tbody id="tablebody">
                                            <c:forEach var="teaching" items="${requestScope.teacher.teaches}">
                                                <tr>
                                                    <td><a href="/OAS/teacher/hod/subjects/detailsubject?subjectId=${teaching.subject.id}">${teaching.subject.name}</a></td>
                                                    <td><a href="/OAS/teacher/hod/classrooms/detailclassroom?classroomId=${teaching.classRoom.id}">${teaching.classRoom.name}</a></td>
                                                    <td><a href="/OAS/teacher/hod/courses/detailcourse?courseId=${teaching.classRoom.course.id}">${teaching.classRoom.course.name}</a></td>
                                                    <td><a href="/OAS/teacher/hod/departments/detaildepartment?departmentId=${teaching.classRoom.course.department.id}">${teaching.classRoom.course.department.name}</a></td>
                                                </tr>
                                            </c:forEach>
                                        </tbody>
                                    </table>
